# encoding: UTF-8
require "rinruby"
require_relative "C:\\D\\DGU\\Repos\\Cassandra\\math_tools.rb"
...
mainresults = File.open("potsdam_regr_summary_step#{step}_norm#{normalize}_dv#{topredict}#{intersection}_#{function}_smooth#{do_smoothing}_#{measure}_nvars#{variables.length}.tsv","w:utf-8")
...
if aggregate
    ...
    variables.each do |variable|
        for cohort in 1..ncohorts do
            ...
            R.eval "m4 = lm(value#{addendum} ~ #{modelformula2}, data = train2)"
            #R.eval "summary(m2)"
            #R.eval "summary(m3)"
    ...
    sum_micro_mae = 0.0
    R.eval "pdf(file='potsdam_predicting_#{step}_dv#{topredict}#{intersection}_#{function}_smooth#{do_smoothing}_nvars#{variables.length}.pdf')"
    ...
    variables.each.with_index do |variable,index|
        ...
        R.eval "plot(dataset2$value#{addendum} ~ dataset2$cohort, ylim = c(min,max), pch=21, col = 'black', bg='black', xlab = '', ylab = '', main = '#{variable}', xaxt = 'n', type= 'b')"
    ...
    o1 = File.open("potsdam_predicting_step#{step}_norm#{normalize}_dv#{topredict}#{intersection}_#{function}_smooth#{do_smoothing}_#{measure}_nvars#{variables.length}_error_separate.tsv","w:utf-8")
    o2 = File.open("potsdam_predicting_step#{step}_norm#{normalize}_dv#{topredict}#{intersection}_#{function}_smooth#{do_smoothing}_#{measure}_nvars#{variables.length}_error_joint_bundle.tsv","w:utf-8")
    o3 = File.open("potsdam_predicting_step#{step}_norm#{normalize}_dv#{topredict}#{intersection}_#{function}_smooth#{do_smoothing}_#{measure}_nvars#{variables.length}_error_joint_categorical.tsv","w:utf-8")
