...
if aggregate
    ...
    variables.each do |variable|
        ...
        if variable == "riskera"
            for c in 0..step2-1
                STDERR.puts "#{vpreds2[variable][c]},#{actual[c]},#{(vpreds2[variable][c]-actual[c]).abs}" 
                sssum += (vpreds2[variable][c]-actual[c]).abs
            end
            STDERR.puts sssum/step2
        end
        ...
        for fold in 1..4 do
            ...
            R.eval "fmae = mean(abs(predsf-actualf))"
            fmae = R.pull "fmae"
            if variable == "riskera"
                STDERR.puts "#{fold} #{fmae}"
...
if individual
    variables = ["riskera", "fortsätta", "försöka", "glömma", "komma", "planera", "slippa", "sluta", "vägra"]
    modelformula = "scale(cohort) + scale(community) + scale(cohort):scale(community) + scale(freq) + trend + scale(cohort):scale(freq) + scale(cohort):trend"
    #modelformula = "scale(cohort)"
    STDERR.puts "Individual analysis"
    R.eval '.libPaths("C:/Users/Sasha/Documents/R/win-library/3.6")'
    R.eval "library(lme4)"
    R.eval "dataset_indiv = read.csv('for_regression_step#{step}_indiv.tsv', sep='\t',header=TRUE)"
    R.eval "names = c('trend','sclass')"
    ...
    variables.each do |variable|
        ...
        #STDERR.puts ind_vpreds2[variable].join(" ")
