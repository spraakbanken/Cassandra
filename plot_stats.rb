...
def fitmodel(values,years,type)
    ...
    if type == "lin"
        R.eval "model = lm(values ~ yearsn)"
    elsif type == "log"
        R.eval "model = lm(values ~ log(yearsn))"
    elsif type == "sqrt"
        R.eval "model = lm(values ~ sqrt(yearsn))"
    end

    R.eval "summodel = summary(model)"
    r2 = R.pull("summodel$r.squared")
    intercept = R.pull("summodel$coefficients[1,1]")
    slope = R.pull("summodel$coefficients[2,1]")
    R.eval "predicted = (predict(model))"
    predicted = R.pull("predicted")
    #if type == "lin"
    #    STDERR.puts predicted.join(" ")
    #end
    return [r2,intercept,slope,predicted]
end
