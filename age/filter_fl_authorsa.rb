...
if type == "age"
    ...
    threshold2 = 10
    years = (2003..2020).to_a
    filehash = {}
    years.each do |year|
        filehash[year] = File.open("#{variable}\\age_corr\\#{corpus}_#{year}.tsv", "w:utf-8")
...
f.each_line.with_index do |line, index|
    if index > 0 
        ...
        if !nickname.include?("Anonym") 
            ...
            if type == "age"
                birthyear = line1[2]
                if !File.exist?("#{variable}\\#{maincorpus}\\#{subcorpus}\\#{nickname2}.tsv")
                    system("ruby korp4a.rb time #{maincorpus}-#{subcorpus} #{variable} #{nickname2}")
                end
                indfile = File.open("#{variable}\\#{maincorpus}\\#{subcorpus}\\#{nickname2}.tsv")
                indfile.each_line.with_index do |iline, iindex|
                    if iindex > 0
                        iline2 = iline.strip.split("\t")
                        if iline2[1].to_i > threshold2 and iline2[0].to_i <= 2020
