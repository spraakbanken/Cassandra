...
oo = File.open("results\\att_dia\\summary_#{years_of_interest.join(",")}_t#{total_threshold}.tsv","w:utf-8")
...
verblist.each do |verb_of_interest|
    ...
    subforums.each do |subforum|
        STDERR.puts "#{years_of_interest} #{verb_of_interest} #{subforum}"
    ...
    o = File.open("results\\att_dia\\familjeliv_#{verb_of_interest}_t#{total_threshold}_#{years_of_interest.join(",")}.tsv","w:utf-8")
    ...
    authorhash.each_pair do |key,value|
        ...
        if total >= total_threshold
            ...
            o.puts "#{years_of_interest.join(",")}\t#{username}\t#{yob}\t\t#{total}\t#{v1abs}\t#{v2abs}\t#{v1rel}\t#{v2rel}"
            totaltotal += total
            v2abstotal += v2abs
        end
